...
class RadioFetcher:
    ...
    def __init__(self):
        ...
        self.playlist_id = "2aKFnLcwzbHRuVSoFpKW2x"
        self.radio_site_url = "https://onlineradiobox.com/us/kuic/playlist/?cs=us.kuic&played=1"
    ...
    def find_song_on_site(self):  # find song name and artist and assign to class variables
        ...
        for song in songs[:40]:
            try:
                song_info = song.a.text.split(" - ")
                song_name = song_info[1]
                song_artist = song_info[0]
                song_artist = re.sub(r'\W+', ' ', song_artist)  # leave only alpha numeric characters in the artist for encoding format
                ...
                if " with " in song_artist:
                    song_artist = song_artist.replace(" with ", " ")  # leave only alpha numeric characters in the artist for encoding format
                ...
                self.new_songs_dict[song_name] = song_artist  # put filtered song info into new songs dictionary
            except Exception:
                try:
                    song_info = song.contents[3].text.split(" - ")  # if the song has no hypertext
                    song_name = song_info[1]
                    song_name = re.sub(r'\n\t', '', song_name)  # remove the \n and \t chars specific to this site path
                    song_artist = song_info[0]
                    song_artist = re.sub(r'\W+', ' ', song_artist)  # leave only alpha numeric characters in the artist for encoding format
                    if " feat " in song_artist:  # remove "feat." and "Featuring"for proper spotify search
                        song_artist = re.sub(r'\W+', ' ', song_artist.replace(" feat ", " "))  # leave only alpha numeric characters in the artist for encoding format
                    if " and " in song_artist:
                        song_artist = re.sub(r'\W+', ' ', song_artist.replace(" and ", " "))  # leave only alpha numeric characters in the artist for encoding format
                    if " with " in song_artist:
                        song_artist = re.sub(r'\W+', ' ', song_artist.replace(" with ", " "))  # leave only alpha numeric characters in the artist for encoding format
                    if " featuring " in song_artist:
                        song_artist = re.sub(r'\W+', ' ', song_artist.replace(" featuring ", " "))  # leave only alpha numeric characters in the artist for encoding format
                    ...
                    pass  # cannot find song info from scrape
...
while True:
    ...
    time.sleep(120)  # add delay (120 seconds) between loops to reduce stress
